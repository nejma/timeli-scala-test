...
class Scala {
  ...
  // Determine the prime numbers up to 'number' inclusively
  // Return a sequence of them
  // We could add a require(number>0, "Negative numbers cannot be prime")
  def slowerPrimes(number: Int): Seq[Int] = {
      def isPrime(n: Int) = {
          if(n <= 1 ) false
          else
